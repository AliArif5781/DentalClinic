...
import heroPortrait from "@assets/generated_images/Woman_with_bright_smile_toothbrush_560cc2f7.png";

export default function LandingHero() {
  const [, setLocation] = useLocation();
  const today = new Date();
  const dayName = today.toLocaleDateString('en-US', { weekday: 'long' });
  const time = today.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-xl">
              <Card className="p-6 bg-card">
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  <div>Monday - Friday<span className="ml-4">9AM - 9PM</span></div>
                  <div>Saturday, Sunday<span className="ml-2">10AM - 6PM</span></div>
                </div>
                <div className="mt-4 pt-4 border-t text-sm">
                  <span className="text-muted-foreground">Today </span>
                  <span className="font-semibold">{dayName}, {time}</span>
                </div>
              </Card>
